    ...
    namespace scene
    {
        namespace quake3
        {
            ...
            */
            ...
            {
                Q3LevelLoadParameter() : defaultLightMapMaterial(video::EMT_LIGHTMAP_M4), defaultModulate(video::EMFN_MODULATE_4X),
                    defaultFilter(video::EMF_BILINEAR_FILTER), patchTesselation(8), verbose(0), startTime(0), endTime(0), mergeShaderBuffer(1),
                    cleanUnResolvedMeshes(1), loadAllShaders(0), loadSkyShader(0), alpharef(1), swapLump(0),
            ...
            */
            ...
            struct SBlendFunc
            {
                SBlendFunc(video::E_MODULATE_FUNC mod) : type(video::EMT_SOLID), modulate(mod), param0(0.f), isTransparent(0)
                {
                }
            ...
            */
            inline static void getBlendFunc(const core::stringc &string, SBlendFunc &blendfunc)
            {
                ...
                static const c8 *funclist[] = {"gl_zero", "gl_one", "gl_dst_color", "gl_one_minus_dst_color", "gl_src_color",
                    "gl_one_minus_src_color", "gl_src_alpha", "gl_one_minus_src_alpha", "gl_dst_alpha", "gl_one_minus_dst_alpha", "gl_src_alpha_sat",
                ...
                {
                    blendfunc.type = video::EMT_ONETEXTURE_BLEND;
                    blendfunc.param0 = video::pack_textureBlendFunc((video::E_BLEND_FACTOR)srcFact, (video::E_BLEND_FACTOR)dstFact,
                        blendfunc.modulate);
            ...
            {
                SModifierFunction() : masterfunc0(UNKNOWN), masterfunc1(UNKNOWN), func(SINUS), tcgen(TEXTURE), rgbgen(IDENTITY), alphagen(UNKNOWN),
                    base(0), amp(1), phase(0), frequency(1), wave(1), x(0), y(0), z(0), count(0)
                {
                }
            ...
            inline core::vector3df getMD3Normal(u32 i, u32 j)
            {
                ...
                return core::vector3df(cosf(lat) * sinf(lng), sinf(lat) * sinf(lng), cosf(lng));
            }
            ...
            inline void getModifierFunc(SModifierFunction &fill, const core::stringc &string, u32 &pos)
            {
                ...
                static const c8 *funclist[] = {"sin", "cos", "square", "triangle", "sawtooth", "inversesawtooth", "noise"};
            ...
            struct SVarGroup
            {
                ...
                u32 isDefined(const c8 *name, const c8 *content = 0) const
                {
                    ...
                    {
                        if (0 == strcmp(Variable[i].name.c_str(), name) && (0 == content || strstr(Variable[i].content.c_str(), content)))
                        {
                            return i + 1;
                        }
                    }
            ...
            */
            inline void getTextures(tTexArray &textures, const core::stringc &name, u32 &startPos, io::IFileSystem *fileSystem,
                video::IVideoDriver *driver
            )
            {
                static const char *extension[] = {".jpg", ".jpeg", ".png", ".dds", ".tga", ".bmp", ".pcx"};
